...
import { useLocation } from "react-router-dom"; // استيراد لقراءة البيانات الممررة عبر الـ Router
...
// البيانات الافتراضية في حال الدخول المباشر بدون الـ Slider
const fallbackVenueData = {
  ...
  price: "$5000 / Night",
  location: { lat: 33.8938, lng: 35.5018, address: "123 Royal Ave, Event City" },
...
export default function ServicesDetails() {
  ...
  // دمج البيانات الممررة من السلايدر (الاسم، الموقع، الصورة، السعة) مع البيانات الافتراضية
  const passedVenue = locationState.state?.venue;

  const venue = {
    name: passedVenue?.name || fallbackVenueData.name,
    // وضع الصورة القادمة من السلايدر كصورة أولى رئيسية
    images: passedVenue?.img ? [passedVenue.img, ...fallbackVenueData.images] : fallbackVenueData.images,
    capacity: passedVenue?.capacity || fallbackVenueData.capacity,
    type: fallbackVenueData.type,
    price: fallbackVenueData.price,
    location: {
      lat: fallbackVenueData.location.lat,
      lng: fallbackVenueData.location.lng,
      address: passedVenue?.location || fallbackVenueData.location.address,
    },
    availableTimes: fallbackVenueData.availableTimes
  };
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: "24px" }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(3, minmax(0, 1fr))", gap: "20px" }}>
            ...
            <div style={{ backgroundColor: "#fafafa", border: "1px solid #e4e4e7", borderRadius: "16px", padding: "16px" }}>
              <h3 style={{ margin: "0 0 12px 0", fontSize: "16px" }}>Details & Specifications</h3>
            ...
            {/* MAP */}
            <div style={{ backgroundColor: "#fafafa", border: "1px solid #e4e4e7", borderRadius: "16px", padding: "16px" }}>
              <h3 style={{ margin: "0 0 12px 0", fontSize: "16px" }}>Location & Map</h3>
              <div style={{ height: "140px", width: "100%", overflow: "hidden", borderRadius: "12px", marginBottom: "10px" }}>
              ...
              <p style={{ fontSize: "13px", color: "#555" }}>📍 {venue.location.address}</p>
              <button onClick={handleDownloadBooklet} style={{ marginTop: "8px", width: "100%", backgroundColor: "#ffffff", border: "1px solid #e4e4e7", padding: "10px", borderRadius: "10px", cursor: "pointer" }}>
                📄 تحميل الكتيب
              </button>
            </div>
            ...
            {/* CALENDAR */}
            <div style={{ backgroundColor: "#fafafa", border: "1px solid #e4e4e7", borderRadius: "16px", padding: "16px" }}>
              <h3 style={{ margin: "0 0 12px 0", fontSize: "16px" }}>Availability Calendar</h3>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "repeat(2, minmax(0, 1fr))", gap: "6px", marginBottom: "12px" }}>
                {venue.availableTimes.map((time) => (
                  <button key={time} onClick={() => setSelectedTime(time)} style={{ padding: "6px 0", fontSize: "11px", borderRadius: "8px", border: "1px solid #e4e4e7", cursor: "pointer", backgroundColor: selectedTime === time ? "#18181b" : "#ffffff", color: selectedTime === time ? "#ffffff" : "#27272a" }}>
                    {time}
                  </button>
                ))}
              </div>

              <button onClick={() => setIsModalOpen(true)} style={{ width: "100%", backgroundColor: "#007bff", color: "#ffffff", border: "none", padding: "10px 0", borderRadius: "10px", fontWeight: "700", cursor: "pointer" }}>
                Proceed to Booking
              </button>
            </div>
          </div>
